...
#define VOLTS_CONSTANT 0.59
#define AMPS_CONSTANT 16.4
...
boolean normalize_data() {
  ...
  for(int i = 0; i < total_samples-2; i++) {
    ...
    voltdata[i] *= VOLTS_CONSTANT;  //Multiply by a constant to convert the digital to actual voltage
    voltdata[i] -= 1;//to get rid of a consistent calculation error
    //        Serial.println(voltdata[i]);
  }

  float max_v = -200.0; //Arbitrary numbers that are greater(lesser) than the peak voltages of ~120 volts
  float min_v = 200.0;
  ...
  //Use the peak voltage to calculate RMS of the sinusoidal wave
...
//  Serial.println(rmsV);
  ...
  if(!first_time){ 
    ...
    //and take the average of the last n samples(See define above)
    avgrmsV = 0.0;
...
//    Serial.println(avgrmsV);  
  ...
  //If it is the first time through, populate the entire array with the first RMS value
  if(first_time){
    for(int i =0; i<MOVING_AVERAGE_NUMBER; i++){
  ...
  //AMPS:
  //Uses the same method as Volts (See above)
  if(!first_time) {
    for(int i = MOVING_AVERAGE_NUMBER-1; i>0; i--){
  ...
  for(int i = 0; i<total_samples-2; i++) {
    ...
    ampdata[i] /= AMPS_CONSTANT;  //Constant between ADC and Amps
  }       

  rmsA = 0;
  //Calculate the current RMS knowing it could be any wave form, not just sinosodial
  for(int i = 0; i<total_samples-2; i++) {
    rmsA += sq(ampdata[i]);
  }
  rmsA /= total_samples-2;
  rmsA = sqrt(rmsA);
  rmsA -= .03; //To get rid of a consistent calculation error
//  Serial.println(rmsA);
...
//As above
  ...
  //Calculate the apparent power by multiplying the rms volts and rms amps
  VA = rmsV * rmsA;
  int VAint = int(VA);
  if(VAint < 2){
    VA = 0.0;
...
//      Serial.print(c);
  ...
  //through the loop, then stop the client:                Still don't understand why, but it works
